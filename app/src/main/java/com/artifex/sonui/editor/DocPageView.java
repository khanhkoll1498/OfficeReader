...
public class DocPageView extends View implements SOPageListener, AnimatableView {
    ...
    public DocPageView(Context var1, SODoc var2) {
        ...
        var4 = new Paint();
        this.z = var4;
        var4.setColor(ContextCompat.getColor(this.getContext(), color.sodk_editor_page_border_color));
        var4.setStyle(Style.STROKE);
        var4.setStrokeWidth((float)Utilities.convertDpToPixel(2.0F));
        var4 = new Paint();
        this.y = var4;
        this.setSelectedBorderColor(ContextCompat.getColor(this.getContext(), color.sodk_editor_selected_page_border_color));
        var4.setStyle(Style.STROKE);
        var4.setStrokeWidth((float)Utilities.convertDpToPixel((float)var1.getResources().getInteger(integer.sodk_editor_selected_page_border_width)));
        if (F == null) {
        ...
        try {
            ...
            if (this.b()) {
                if (this.getParent() instanceof DocListPagesView) {
                    var9 = Math.min(Math.max(this.g.left - this.mPageRect.left, 0), 0);
                    var10 = var8;
                } else {
                    var10 = Math.min(Math.max(this.mPageRect.right - this.g.right, 0), 0);
                    var9 = var7;
                }
            }
            ...
            Double.isNaN(var11);
            Double.isNaN(var11);
            this.d = var15.a(var10, var11 * var13, (double)this.mRenderOrigin.x, (double)this.mRenderOrigin.y, this.j, (SOBitmap)null, new o() {
            ...
            Log.e("TAGGGGGGGGGGGGGGGGGGGGG", "a: " + exception.getMessage() );
    ...
    private void getDataLeakHandlers() {
        ...
        try {
            ...
            str = String.format("getDataLeakHandlers() experienced unexpected exception [%s]", new Object[]{"ExceptionInInitializerError"});
            Log.e("DocPageView", str);
        } catch (LinkageError e3) {
            str = String.format("getDataLeakHandlers() experienced unexpected exception [%s]", new Object[]{"LinkageError"});
            Log.e("DocPageView", str);
        } catch (SecurityException e4) {
            str = String.format("getDataLeakHandlers() experienced unexpected exception [%s]", new Object[]{"SecurityException"});
            Log.e("DocPageView", str);
            ...
            if (this.isShown()) {
                if (this.mPage != null) {
                    ...
                    if (var2 == null) {
                        ...
                        Point var4 = this.E;
                        if (var4 == null || var5 == null || var4.x == var5.x && this.E.y == var5.y) {
                            var6 = this.r;
                        } else {
                            var6 = new Rect();
                            this.getLocalVisibleRect(var6);
